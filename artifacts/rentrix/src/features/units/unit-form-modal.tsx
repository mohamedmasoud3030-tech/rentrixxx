    ...
    >
      ...
      >
        ...
        <label className="grid gap-2 text-sm font-bold">
          رقم الوحدة
          <Input hasError={Boolean(errors.unit_number)} aria-describedby={errors.unit_number ? 'unit-number-error' : undefined} {...form.register('unit_number')} />
          {fieldError('unit-number-error', errors.unit_number?.message)}
        </label>
        <label className="grid gap-2 text-sm font-bold">
          الدور
        ...
        <label className="grid gap-2 text-sm font-bold">
          الحالة
          <Select hasError={Boolean(errors.status)} aria-describedby={errors.status ? 'unit-status-error' : undefined} {...form.register('status')}>
            {unitStatusValues.map((status) => <option key={status} value={status}>{unitStatusLabels[status]}</option>)}
          </Select>
          {fieldError('unit-status-error', errors.status?.message)}
        </label>
        <label className="grid gap-2 text-sm font-bold">
          قيمة الإيجار
          <Input hasError={Boolean(errors.rent_amount)} aria-describedby={errors.rent_amount ? 'unit-rent-error' : undefined} type="number" step="0.01" min="0" {...form.register('rent_amount')} />
          {fieldError('unit-rent-error', errors.rent_amount?.message)}
        </label>
        ...
        <div className="safe-bottom-overlay -mx-4 flex flex-col-reverse gap-3 border-t border-border/60 px-4 pt-4 sm:mx-0 sm:flex-row sm:justify-end sm:border-0 sm:px-0 sm:pb-0 md:col-span-2">
          ...
          <Button type="submit" isLoading={isSubmitting}>حفظ الوحدة</Button>
        </div>
      </form>
    </Modal>
  );
