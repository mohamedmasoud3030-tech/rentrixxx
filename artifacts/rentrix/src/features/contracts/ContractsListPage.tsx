...
import { AlertTriangle, CalendarClock, CheckSquare, ChevronDown, ChevronUp, Download, Edit, Eye, FileText, Plus, Search, Square, Trash2, WalletCards } from 'lucide-react';
...
export function ContractsListPage() {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      {!contractsQuery.isLoading && !contractsQuery.isError && filteredContracts.length ? <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead className="w-12"><button onClick={bulk.toggleAll} aria-label="تحديد الكل">{bulk.allSelected ? <CheckSquare className="size-4" /> : <Square className="size-4" />}</button></TableHead><TableHead>تفاصيل</TableHead><TableHead>رقم العقد</TableHead><TableHead>المستأجر</TableHead><TableHead>الوحدة</TableHead><TableHead>البداية</TableHead><TableHead>النهاية</TableHead><TableHead>الإيجار</TableHead><TableHead>الحالة</TableHead><TableHead>إجراءات</TableHead></TableRow></TableHeader><TableBody>
      {filteredContracts.map((contract) => { const isExpanded = expandedId === contract.id; const daysUntilEnd = getDaysUntilEnd(contract); return <Fragment key={contract.id}><TableRow><TableCell><button onClick={() => bulk.toggleOne(contract.id)} aria-label={`تحديد ${getContractNumber(contract)}`}>{bulk.isSelected(contract.id) ? <CheckSquare className="size-4" /> : <Square className="size-4" />}</button></TableCell><TableCell><Button variant="ghost" className="min-h-9 px-3" onClick={() => setExpandedId((current) => (current === contract.id ? null : contract.id))}>{isExpanded ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}</Button></TableCell><TableCell>{getContractNumber(contract)}</TableCell><TableCell>{contract.people?.full_name ?? '—'}</TableCell><TableCell>{contract.units?.unit_number ?? contract.properties?.title ?? '—'}</TableCell><TableCell>{formatContractDate(companySettings, contract.start_date)}</TableCell><TableCell>{formatContractDate(companySettings, contract.end_date)}</TableCell><TableCell>{formatContractMoney(companySettings, contract.rent_amount)}</TableCell><TableCell><StatusBadge tone={contractStatusTone[contract.status]}>{contractStatusLabels[contract.status]}</StatusBadge></TableCell><TableCell><div className="flex gap-2"><Button variant="secondary" className="min-h-9 px-3" asChild><Link to="/contracts/$contractId" params={{ contractId: contract.id }}><Eye className="size-4" /></Link></Button><Button variant="secondary" className="min-h-9 px-3" asChild><Link to="/contracts/$contractId/edit" params={{ contractId: contract.id }}><Edit className="size-4" /></Link></Button><Button variant="danger" className="min-h-9 px-3" onClick={() => deleteMutation.mutate(contract.id)} disabled={deleteMutation.isPending}><Trash2 className="size-4" /></Button></div></TableCell></TableRow>{isExpanded ? <TableRow><TableCell colSpan={10} className="bg-muted/30 p-4"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5"><DetailBox label="قيمة الإيجار"><p className="text-lg font-black" dir="ltr">{formatContractMoney(companySettings, contract.rent_amount)}</p><p className="text-muted-foreground">دورة السداد: {paymentCycleLabels[contract.payment_cycle]}</p><p className="text-xs text-muted-foreground">لا يتوفر حاليًا رصيد مدفوع/مستحق دقيق على مستوى العقد من البيانات المحملة.</p></DetailBox><DetailBox label="الفترة"><p>{formatContractDate(companySettings, contract.start_date)} ← {formatContractDate(companySettings, contract.end_date)}</p>{daysUntilEnd !== null && daysUntilEnd >= 0 ? <p className="text-amber-700">ينتهي خلال {daysUntilEnd} يوم</p> : null}</DetailBox><DetailBox label="إجراءات إضافية"><Button variant="secondary" disabled={renewMutation.isPending} onClick={() => { const start = parseContractDisplayDate(contract.end_date); if (!start) return; const nextStart = new Date(start); nextStart.setDate(nextStart.getDate() + 1); const nextEnd = new Date(nextStart); nextEnd.setFullYear(nextEnd.getFullYear() + 1); renewMutation.mutate({ contractId: contract.id, payload: { new_start: formatLocalIsoDate(nextStart), new_end: formatLocalIsoDate(nextEnd), new_amount: contract.rent_amount } }); }}>تجديد العقد (سنة)</Button></DetailBox></div></TableCell></TableRow> : null}</Fragment>; })}
      </TableBody></Table></div> : null}
      {!contractsQuery.isLoading && contractsQuery.isError ? <div className="p-6"><EmptyState title="تعذر تحميل العقود" description="حدث خطأ أثناء تحميل البيانات. حاول مرة أخرى." action={<Button onClick={() => contractsQuery.refetch()}>إعادة المحاولة</Button>} /></div> : null}
