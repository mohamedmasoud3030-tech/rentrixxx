...
export function ContractDetailView({ contract, companySettings }: Readonly<{ contract: ContractDetail; companySettings: CompanySettingsContract }>) {
  ...
  const exportContractPdf = () => exportContractToPdf(contract, { settings: { general: { company: { name: 'Rentrix' } } }, contracts: [contract], tenants: toPdfTenant(contract.people) ? [toPdfTenant(contract.people)!] : [], units: toPdfUnit(contract.units, contract.property_id) ? [toPdfUnit(contract.units, contract.property_id)!] : [], properties: toPdfProperty(contract.properties) ? [toPdfProperty(contract.properties)!] : [] });

  return <div className="space-y-6">{/* trimmed for brevity in refactor */}
    <div className="flex flex-wrap items-start justify-between gap-3"><div className="space-y-1"><p className="text-sm font-black text-primary">العقد رقم #{contract.id.slice(0, 8)}</p><h2 className="text-3xl font-black">تفاصيل العقد</h2><p className="text-sm text-muted-foreground">عرض كامل للعقد وسجل مراحله.</p></div><div className="flex gap-2"><Button variant="secondary" asChild><Link to="/contracts"><ArrowRight className="me-2 size-4" />العودة</Link></Button><Button variant="secondary" onClick={openRenewalDialog} disabled={!renewalAllowed}><RefreshCw className="me-2 size-4" />تجديد</Button><Button variant="secondary" onClick={exportContractPdf}>تصدير PDF</Button><Button asChild><Link to="/contracts/$contractId/edit" params={{ contractId: contract.id }}><Edit className="me-2 size-4" />تعديل</Link></Button></div></div>
