...
export function MaintenancePage() {
  ...
  const onSubmit = (values: MaintenanceFormValues) => {
    createMutation.mutate({ ...values, status: 'open', assigned_to: null, cost: 0, resolved_at: null, unit_id: values.unit_id ?? null });
  };

  return <div className='space-y-6' dir='rtl'>
    <Card><CardHeader><CardTitle>طلبات الصيانة</CardTitle></CardHeader><CardContent className='space-y-2'>
      <div className='flex gap-2'>
        <select className='rounded border px-2' value={status} onChange={(e)=>setStatus(e.target.value as typeof status)}><option value='all'>كل الحالات</option><option value='open'>مفتوح</option><option value='in_progress'>قيد التنفيذ</option><option value='resolved'>تم الحل</option><option value='closed'>مغلق</option></select>
        <select className='rounded border px-2' value={propertyFilterId} onChange={(e)=>setPropertyFilterId(e.target.value)}><option value=''>كل العقارات</option>{properties?.rows.map((p) => <option key={p.id} value={p.id}>{p.title}</option>)}</select>
      </div>
      <DataTable rows={data} keyOf={(r) => r.id} columns={[{ key: 'title', header: 'العنوان', render: (r) => r.title }, { key: 'status', header: 'الحالة', render: (r) => r.status }, { key: 'priority', header: 'الأولوية', render: (r) => r.priority }]} empty={<EmptyState title='لا توجد طلبات صيانة' description='ابدأ بإضافة طلب جديد لهذا العقار.' />} />
      <form className='grid gap-2' onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <select className='rounded border px-2 py-2' {...form.register('unit_id')}><option value=''>بدون وحدة</option>{units.map((u) => <option key={u.id} value={u.id}>{u.unit_number}</option>)}</select>
        ...
        <FormActions submitLabel='إضافة طلب صيانة' isSubmitting={createMutation.isPending} onSubmit={form.handleSubmit(onSubmit)} />
      </form>
    </CardContent></Card>
  </div>;
}
