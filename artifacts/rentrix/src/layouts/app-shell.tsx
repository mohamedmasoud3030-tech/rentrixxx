    ...
    <div className="min-h-screen bg-background text-foreground" dir="rtl">
      <aside className={cn('fixed inset-y-0 right-0 z-30 hidden overflow-hidden border-l border-sidebar-border bg-sidebar text-sidebar-foreground shadow-sidebar transition-all lg:flex lg:flex-col', sidebarCollapsed ? 'w-20' : 'w-72')}>
        ...
        <div className="flex h-24 items-center gap-3 border-b border-white/10 px-5">
          ...
          {!sidebarCollapsed ? (
            <div>
              <p className="text-xl font-black text-white">Rentrix</p>
              <p className="text-xs font-bold text-sidebar-foreground">إدارة العقارات</p>
            </div>
          ) : null}
        </div>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-black text-sidebar-foreground transition hover:bg-sidebar-accent hover:text-white [&.active]:bg-primary [&.active]:text-primary-foreground [&.active]:shadow-[0_10px_28px_-14px_hsl(var(--primary))]"
        ...
        <header className="sticky top-0 z-20 border-b border-border bg-background/88 backdrop-blur-xl">
          <div className="flex h-20 items-center justify-between px-6">
            <div className="flex items-center gap-4">
              <Button variant="ghost" className="size-10 px-0" onClick={toggleSidebar} aria-label="طي القائمة"><Menu className="size-5" /></Button>
              <div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground"><span>الرئيسية</span><ChevronLeft className="size-3" /><span>{pageTitle}</span></div>
                <h1 className="mt-1 text-3xl font-black tracking-tight">{pageTitle}</h1>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="rounded-2xl border border-border bg-card px-4 py-2 text-xs text-muted-foreground"><span className="font-bold text-foreground">{syncStatus}</span>{lastSyncedAt ? ` · ${new Date(lastSyncedAt).toLocaleTimeString('ar')}` : ''}</div>
              <Button variant="secondary" className="size-10 px-0" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label="تبديل الوضع">{theme === 'dark' ? <Sun className="size-5" /> : <Moon className="size-5" />}</Button>
