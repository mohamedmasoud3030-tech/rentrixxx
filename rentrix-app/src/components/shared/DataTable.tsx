...
export function DataTable<T>({ rows, columns, keyOf, empty }: { rows: T[]; columns: Array<Column<T>>; keyOf: (row: T) => string; empty?: ReactNode }) {
  ...
  return (
    <div className="mobile-scroll-x rounded border">
      <table className="w-full min-w-max text-start">
        <thead><tr>{columns.map((c) => <th key={c.key} className="whitespace-nowrap border-b px-3 py-2 text-sm font-bold">{c.header}</th>)}</tr></thead>
