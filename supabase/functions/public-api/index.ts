...
Deno.serve(async (req) => {
  ...
  const lookupIdempotencyResult = async (operationName: string): Promise<unknown | null> => {
  ...
  const storeIdempotencyResult = async (operationName: string, responsePayload: JsonObject): Promise<void> => {
    ...
    if (error) {
      console.error('[public-api] failed to store idempotency result', { operationName, error: error.message });
    }
  };
  ...
  try {
    ...
    if (method === 'POST' && path === '/receipts') {
      ...
      const cachedResponse = await lookupIdempotencyResult(operationName);
