...
| Communications | Not in current app | `CommunicationHub.tsx`, `WhatsAppComposerModal.tsx`, `whatsappService.ts` | WhatsApp composer, templates, logs, bulk reminders | Follow `docs/COMMUNICATIONS_SCHEMA_DESIGN.md`; add schema/services first, then safe UI after phone/log foundations. |
