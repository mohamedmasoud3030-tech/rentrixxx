...
**الحالة:** P0/P1 (عدا Bank Reconciliation) مُطبَّقة فعلياً على live DB `nnggcnpcuomwfuupupwg` بتاريخ 2026-06-28 — انظر `docs/MIGRATION_IMPLEMENTATION_REPORT.md` للتفاصيل والتصحيحات
...
#### P0-A: إصلاح `record_invoice_payment_atomic` — ⚠️ تبيّن أنه محلول مسبقاً (no-op)

**تحديث 2026-06-28:** قبل التطبيق على live DB، تأكدنا أن هذه المشكلة **محلولة بالفعل** عبر PR #896
(`20260615000100_fix_invoice_payment_account_resolution`) وتم تشديدها أكثر في PR #911. الدالة الحية
أكثر أماناً من الحل المقترح هنا (تتحقق من وجود الأعمدة، تدعم fallback بالـ regex، وتعمل
`RAISE EXCEPTION` عند تطابق أكثر من حساب بدل اختيار واحد بصمت بـ `LIMIT 1`، وتضبط
`search_path`). تطبيق الـ migration الأصلية هنا كان سيُراجع للخلف. تم تحويل ملف الـ migration إلى
no-op — انظر `docs/ai/CURRENT_EXECUTION_CONTEXT.md` قسم "Known Contradictions Resolved".

**المشكلة (تاريخياً، قبل PR #896):**
...
**المكونات المطلوبة:**
- [x] Migration: `add_cost_centers` — مُطبَّقة live (مُصحَّحة: text ids بدل uuid، idempotent)
- [x] RLS policies بنمط `is_app_user()` — مؤكَّدة live
- [x] `costCenterService.ts`
- [x] `useCostCenters.ts`
- [x] صفحة CRUD بسيطة في Settings
- [x] ربط `cost_center_id` في نموذج المصروفات
- [ ] تحديث `rpt_income_statement` لتصفية حسب cost center — لم يُنفَّذ بعد، متبقٍّ
...
**المكونات المطلوبة:**
- [x] Migration: `add_rpt_cash_flow` — مُطبَّقة live (مُصحَّحة: أُضيف `search_path`)
- [x] `financialReportsService.ts` — `getCashFlowStatementReport()`
- [x] مكون عرض Cash Flow في قسم Reports/Statements
- [ ] CSV export مخصص لتقرير Cash Flow — متبقٍّ (التقارير الأخرى لديها CSV، هذا لم يُنفَّذ)
...
**المكونات المطلوبة:**
- [x] Migration: `add_payment_terms` — مُطبَّقة live (مُصحَّحة: text ids، idempotent)
- [x] CRUD في Settings
- [x] ربط في نموذج العقد (اختيار `payment_terms_id`)
- [ ] `generate_invoices_from_active_contracts` يحترم جدول الدفعات — **غير منفَّذ**، العقد يخزّن
      ويختار payment terms لكن لا يُولِّد جدول دفعات فعلي. موثَّق كـ "Future Narrow Enhancement"
      في `docs/MIGRATION_IMPLEMENTATION_REPORT.md`
...
**المكونات المطلوبة:**
- [x] Migration: `add_vat_support` — مُطبَّقة live (مُصحَّحة: أُضيف `search_path`، `tax_amount`
      موجود مسبقاً فلم يُكرَّر)
- [x] إعدادات VAT في Settings
- [x] حساب VAT تلقائي عند إنشاء الفاتورة
- [x] عرض VAT في الفاتورة (المعروضة/المطبوعة)
- [x] تقرير `rpt_vat_return` — ملخص للفترة المحددة (ليس مُقسَّماً شهري/ربع سنوي تلقائياً بعد)
- [x] تحديث `database.ts` بعد Migration
...
```
✅ منجَز (2026-06-28، مُطبَّق live على nnggcnpcuomwfuupupwg):
  [P0-A] إصلاح record_invoke_payment_atomic — تبيّن أنه محلول مسبقاً، no-op
  [P0-B] Cost Centers — schema + service + UI (cost-center filtering للتقارير متبقٍّ)
  [P1-D] VAT Support — migration + settings + invoice display
  [P1-C] Payment Terms — template + contract link (جدول دفعات فعلي متبقٍّ)
  [P1-A] Cash Flow Report — RPC + UI component (CSV export متبقٍّ)
...
**المراجع:** `docs/FINAL_PRODUCT_BLUEPRINT.md`, `docs/RENTRIX_MASTER_PLAN.md`, تحليل ERPNext GAP, `docs/MIGRATION_IMPLEMENTATION_REPORT.md`
**الحالة:** P0/P1 مُطبَّقة live عدا Bank Reconciliation (P1-B) وبعض الربط المتبقي (تقرير cost-center للدخل، جدول دفعات فعلي، CSV لـ Cash Flow) — موثَّق أعلاه تحت كل بند
